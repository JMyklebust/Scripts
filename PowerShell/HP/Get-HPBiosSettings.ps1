...
#Get BIOS Verison
$BIOSVersion = (Get-CimInstance -ClassName CIM_BIOSElement -CimSession $Session).SMBIOSBIOSVersion 
#End session connection as data has been collected
...
@("#Date:      $(Get-Date -Format s)","#Hostname:  $($ComputerInfo.Name)", "#Model:     $($ComputerInfo.Model)","#BIOS Ver.: $BIOSVersion","") +  (Get-Content $OutFile -Encoding Ascii) | Set-Content $OutFile -Encoding Ascii
